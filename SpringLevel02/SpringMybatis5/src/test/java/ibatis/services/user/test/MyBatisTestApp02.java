...
public class 	MyBatisTestApp02 {
	...
	public static void main(String[] args) throws Exception{
		...
		System.out.println(":: 2. 01로 끝나는 userId User(SELECT)  ? ");
		 list = session.selectList("User02.getUser02",user);	//인자값이 user 객체면 parameterType="user",
		 														//인자값이 "user"로 주입하면 parameterType="string"
		for (int i =0 ;  i < list.size() ; i++) {
			System.out.println( "<"+ ( i +1 )+"> 번째 회원.."+ list.get(i).toString() );
		}
		System.out.println("\n");
		...
		System.out.println(":: 5. 01로 끝나는 userId User(SELECT)  ? ");
		 list = session.selectList("User02.getUser04",user); 	//인자값이 user 객체면 parameterType="user",
		 														//인자값이 "user"로 주입하면 parameterType="string"
		for (int i =0 ;  i < list.size() ; i++) {
			System.out.println( "<"+ ( i +1 )+"> 번째 회원.."+ list.get(i).toString() );
		}
