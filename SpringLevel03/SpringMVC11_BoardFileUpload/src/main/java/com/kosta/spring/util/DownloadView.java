...
public class DownloadView extends AbstractView{
 ...
 protected void renderMergedOutputModel(Map<String, Object> map,
  ...
  String orgfilename=request.getParameter("orgfilename"); // 다운시에는 orgfilename 가져와야함.
  String newfilename=request.getParameter("newfilename");
  
  //업로드 경로가 저장된 파일 객체
  File file=new File(path+newfilename); //실질적으로 다운받는 것은 newfilename
  ...
  // 파일 다운로드 
    ...
    response.setHeader("Content-Disposition", 
      "attachment; fileName="
    +new String(orgfilename.getBytes("UTF-8"),"8859_1")); // 다운시에는 orgfilename 가져와야함.
